...
export function CopyButton({ getText, class: className, titleKey, variant = "icon" }: CopyButtonProps) {
  ...
  if (variant === "label") {
    return (
      <button
        onClick={handleCopy}
        class={`btn-primary flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium ${
          state === "ok" ? "!bg-primary" : state === "fail" ? "!bg-red-500" : ""
        } ${className || ""}`}
      >
    ...
    <button
      ...
      class={`p-1.5 rounded-md transition-colors duration-150 ${
        state === "ok"
          ...
          : state === "fail"
            ? "text-red-500"
            : "text-[var(--text-tertiary)] hover:text-primary hover:bg-primary/10"
      } ${className || ""}`}
